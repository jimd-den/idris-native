...
- **ADT Lowering**: Data types are lowered to LLVM structs `{ i64 tag, [N x i64] fields }` with global constructor functions.
- **Pattern Matching**: Implemented using LLVM's `switch` instruction for efficient tag and literal dispatch.
